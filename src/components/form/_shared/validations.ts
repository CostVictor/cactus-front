...
export const genericValidations = {
  ...
  minLength: (min: number) => (value) => value.length >= min || `Este campo deve ter pelo menos "${min}" caracteres`,
...
export const typeValidations = {
  email: () => (value) => /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(value) || 'O endereço de e-mail é inválido',
  ...
  password: () => ({
    minLength: (value) => value.length >= 10 || 'A senha deve ter pelo menos 10 caracteres',
